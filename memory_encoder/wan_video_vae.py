...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        ...
        for layer in self.residual:
            if check_is_instance(layer,
                                 CausalConv3d) and feat_cache is not None:
                idx = feat_idx[0]
                cache_x = x[:, :, -CACHE_T:, :, :].clone()
...
class Encoder3d(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        ...
        for layer in self.middle:
            if check_is_instance(layer,
                                 ResidualBlock) and feat_cache is not None:
                x, feat_cache, feat_idx = layer(x, feat_cache, feat_idx)
            else:
                x = layer(x)
...
class VideoVAE_(nn.Module):
    ...
    def clear_cache(self):
        ...
        # self.register_buffer("_feat_map", torch.tensor([None] * self._conv_num, dtype=torch.float32))  # 按实际shape初始化
        # self.register_buffer("_conv_idx", torch.tensor([0], dtype=torch.int64))
        self._conv_idx = [0]
        self._feat_map = [None] * self._conv_num
        # cache encode
        self._enc_conv_num = count_conv3d(self.encoder)
        # self.register_buffer("_enc_feat_map", torch.tensor([None] * self._enc_conv_num, dtype=torch.float32))  # 按实际shape初始化
        # self.register_buffer("_enc_conv_idx", torch.tensor([0], dtype=torch.int64))
        self._enc_conv_idx = [0]
...
class WanVideoVAE(nn.Module):
    ...
    def __init__(self, z_dim=16, mean=None, std=None):
        super().__init__()
    ...
    def tiled_decode(self,
        ...
        for h, h_, w, w_ in tqdm(tasks,
                                 desc="VAE decoding",
                                 disable=not verbose):
            hidden_states_batch = hidden_states[:, :, :, h:h_,
                                                w:w_].to(computation_device)
            hidden_states_batch = self.model.decode(hidden_states_batch,
                                                    self.scale).to(data_device)
    ...
    def tiled_encode(self,
        ...
        weight = torch.zeros((1, 1, out_T, H // self.upsampling_factor,
                             ...
                             device=data_device)
        values = torch.zeros(
            (1, self.z_dim, out_T, H // self.upsampling_factor,
             W // self.upsampling_factor),
            dtype=video.dtype,
            device=data_device)
        ...
        for h, h_, w, w_ in tqdm(tasks,
                                 desc="VAE encoding",
                                 disable=not verbose):
            hidden_states_batch = video[:, :, :, h:h_,
                                        w:w_].to(computation_device)
            hidden_states_batch = self.model.encode(hidden_states_batch,
                                                    self.scale).to(data_device)
            ...
            mask = self.build_mask(
                ...
                border_width=((size_h - stride_h) // self.upsampling_factor,
                              (size_w - stride_w) //
                              self.upsampling_factor)).to(dtype=video.dtype,
                                                          device=data_device)
    ...
    def encode(self,
        ...
        for video in videos:
            ...
            if tiled:
                tile_size = (tile_size[0] * self.upsampling_factor,
                             tile_size[1] * self.upsampling_factor)
                tile_stride = (tile_stride[0] * self.upsampling_factor,
                               tile_stride[1] * self.upsampling_factor)
                hidden_state = self.tiled_encode(video, device, tile_size,
                                                 tile_stride, verbose)
            else:
    ...
    def decode(self,
        ...
        for hidden_state in hidden_states:
            ...
            if tiled:
                video = self.tiled_decode(hidden_state, device, tile_size,
                                          tile_stride, verbose)
            else:
...
class VideoVAE38_(VideoVAE_):
    ...
    def __init__(self,
        ...
        self.decoder = Decoder3d_38(dec_dim, z_dim, dim_mult, num_res_blocks,
                                    attn_scales, self.temperal_upsample,
                                    dropout)
...
class WanVideoVAE38(WanVideoVAE):
    ...
    def __init__(self, z_dim=48, dim=160, mean=None, std=None):
        ...
        # init model
        self.model = VideoVAE38_(z_dim=z_dim,
                                 dim=dim).eval().requires_grad_(False)
